<script>
	...
	presenze.forEach((item, idx) => {
		presenze[idx].presenza['full_name'] = presenze[idx].presenza['cognome'].concat(
			' ',
			presenze[idx].presenza['nome']
		);
	...
	if (helper.is_tutor(data) && !helper.is_admin(data))
		pcto = [
			...pcto.filter(
				(stage) =>
					(stage.idTutor == id_utente || stage.tutor_aziendale == helper.user_name(data)) &&
					stage.contabilizzato == false
			)
		];
	...
	$: {
		...
		if (selected_stage[0]) {
			if (helper.user_ruolo(data) != 'STUDENTE') pcto_studenti = selected_stage[0].svoltoDa;
			else pcto_studenti = selected_stage[0].svoltoDa.filter((item) => item.id == id_utente);
		}
	}
	...
	const form_schema = yup.object().shape({
		...
		studente: yup.number().min(0, 'Studente necessario'),
		...
		oraInizio: yup
			...
			.test(
				'minore',
				"L'orario d'entrata deve essere precedente a quello d'uscita",
				(value, textContext) => {
		...
		oraFine: yup
			...
			.test(
				'maggiore',
				"L'orario d'uscita deve essere successivo a quello d'entrata",
				(value, textContext) => {
	...
	async function handleSubmit() {
		try {
			...
			errors = {};
			if (form_values.studente == 0 && modal_action == 'create')
				//TUTTI GLI STUDENTI
			...
			logger.error(
				`Errori nella validazione del form presenze. Oggetto: ${JSON.stringify(
					form_values
...
{#if tipo_utente == 'STUDENTE'}
	<p class="ore-pcto">
		...
		<button class="btn position-relative"
			>Registrate <span class="badge bg-green ms-2">{totale_ore_pcto.registrate}</span></button
		>
		<button class="btn position-relative"
			>Totali <span class="badge bg-yellow ms-2">{totale_ore_pcto.totali}</span></button
		>
	</p>
{/if}
...
{#if !data.session.mobile}
	<Table
		columns={[
			...
			{ name: 'as', type: 'string', display: 'as', search: true },
			...
			<div class="modal-content">
				...
				<div class="modal-body">
						...
						<div class="col-lg-6">
							...
							<div class="mb-3">
								{#if pcto_studenti.length >= 1}
									...
									<select
										class="form-select"
										class:is-invalid={errors.studente}
										name="studente"
										bind:value={form_values.studente}
									>
					...
					<div class="row">
						...
						<div class="col-lg-4">
							<InputTime label="Uscita" name="oraFine" {errors} bind:val={form_values.oraFine} />
						</div>
					</div>
				</div>
